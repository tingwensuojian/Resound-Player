...
- **StreamingServer（`:38764`）** ✅ 启动正常，IOWorker + N-API 在 macOS 上完整验证通过
- **大文件 Range 限制 1MB** ✅ 已在 `StreamingServer.js` 中落地
- **UI 加载反馈** ✅ `PlayerBar.vue` 播放按钮有 Loader2 旋转动画

### 与此方案相关的附加修复（扫描导入流程）

本方案的 Range 流式播放依赖本地音乐扫描流程正确导入歌曲，扫描流程在本次集成中发现以下问题并修复：

#### ScannerWorker IPC 消息大小溢出
| 问题 | 修复 |
|------|------|
| 389 个文件的元数据+封面数据通过 IPC 一次性发送，JSON 字符串超过 V8 限制 | 改为每 20 条一批发送 |
| 封面图片数据（每张数 MB）即使 20 条一批仍超限 | ScannerWorker 直接写入本地封面缓存目录，IPC 只发元数据 |

#### covercache:// 协议 URL 尾部斜杠
| 问题 | 根因 | 修复 |
|------|------|------|
| 封面显示为破碎图片 | Electron 标准协议自动追加 `/` 到路径末尾 | `cacheKey` 正则去掉尾部斜杠 |
...
3. 确认：
   - [x] 点击播放后播放按钮立即显示旋转动画
   - [x] 1-3 秒内听到声音
   - [x] StreamingServer Range 流式读取正常
   - [x] 整体 UI 不卡顿