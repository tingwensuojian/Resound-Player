...
import { getIntelligenceList, getPlaylistTrackAll, getSongDetail, trashPersonalFm, toggleSongLike, toggleDjSubscribe } from '../api/music';
    ...
    switch (command.type) {
      ...
      case 'toggleLike': {
        ...
        if (userStore.state.loginMode !== 'cookie' && userStore.state.loginMode !== 'qr') {
          ...
          loginModalStore.showGlobalToast('?????????????????????? Cookie ??', 'warning', 5000);
        ...
        const _isPodcast = _track.source === 'podcast' && !!_track.podcast?.rid;


        const _next = _isPodcast


          ? !userStore.state.subscribedDjIds.includes(_track.podcast.rid)


          : !userStore.state.likedSongIds.includes(Number(_track.id));


        try {


          if (_isPodcast && _track.podcast?.rid) {


            await toggleDjSubscribe({ rid: _track.podcast.rid, subscribe: _next, cookie: userStore.state.loginCookie || undefined });


            const rid = _track.podcast.rid;


            if (_next) {


              if (!userStore.state.subscribedDjIds.includes(rid)) userStore.state.subscribedDjIds = [...userStore.state.subscribedDjIds, rid];


            } else {


              userStore.state.subscribedDjIds = userStore.state.subscribedDjIds.filter((id) => id !== rid);
            ...
            await toggleSongLike({ id: Number(_track.id), like: _next, uid: userStore.state.profile?.userId, cookie: userStore.state.loginCookie || undefined });


            const id = Number(_track.id);
            ...
            if (_next) {
              ...
              userStore.state.likedSongIds = userStore.state.likedSongIds.filter((sid) => sid !== id);
